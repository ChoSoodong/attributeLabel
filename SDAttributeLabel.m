...
#import "SDAttributeLabel.h"

@interface SDAttributeLabel()
/* 布局管理者 */
...
@property(nonatomic, strong)NSTextStorage *textString;


/* 点击类型 */
@property(nonatomic, assign)SDAttributeLabelStyle ClickStyle;
...
@end
@implementation SDAttributeLabel


static NSString *SDRange = @"sdrange";
static NSString *SDColor = @"sdcolor";

-(void)setHightLightLabelColor:(UIColor *)hightLightColor forStyle:(SDAttributeLabelStyle)style{
    switch (style) {
        case SDAttributeLabelStyleLink:
        {
            self.linkHightColor = hightLightColor;
            [self prepareText];
        }
            break;
        case SDAttributeLabelStyleTopic:
        {
            self.topicHightColor = hightLightColor;
            [self prepareText];
        }
            break;
        case SDAttributeLabelStyleAgreement:
        {
            self.agreementHightColor = hightLightColor;
            [self prepareText];
        }
            break;
        case SDAttributeLabelStyleUser:
        {
            self.userHightColor = hightLightColor;
            [self prepareText];
        }
            break;
        case SDAttributeLabelStylePhoneNumber:
        {
...
-(void)setup{
    ...
    _textString = [NSTextStorage new];
    _layout = [NSLayoutManager new];
    _textContainer = [NSTextContainer new];
    _normalColor = [UIColor colorWithRed:162.0/255 green:162.0/255  blue:162.0/255  alpha:162.0/255];
    _hightLightBackgroundColor = [UIColor colorWithWhite:0.7 alpha:0.2];
...
-(void)drawRect:(CGRect)rect{
    ...
    if (self.selectedRange.length != 0) {
        ...
        // 2.0.确定颜色
        UIColor *selectedColor = self.isSelected ? self.hightLightBackgroundColor : [UIColor clearColor];
        
        // 2.1.设置颜色
        [self.textString addAttribute:NSBackgroundColorAttributeName value:selectedColor range:self.selectedRange];
    ...
    // 2.绘制字形
    // 需要绘制的范围
    NSRange range = NSMakeRange(0, self.textString.length);
...
-(void)prepareTextSystem{
    ...
    // 1.将布局添加到storeage中
    [self.textString addLayoutManager:self.layout];
...
-(void)prepareText{
    ...
    attr = @{
             ...
             NSForegroundColorAttributeName : self.normalColor
             };
    
    [attrStringM setAttributes:attr range:NSMakeRange(0, attrStringM.length)];
    
    // 4.设置TextString的内容
    [self.textString setAttributedString:attrStringM];
    ...
    for (NSValue *value in linkRanges) {
        ...
        [self.textString addAttribute:NSForegroundColorAttributeName value:self.linkHightColor range:range];
    }
    ...
    for (NSValue *value in phoneNumberRanges) {
        NSRange range;
        [value getValue:&range];
        [self.textString addAttribute:NSForegroundColorAttributeName value:self.PhoneNumberHightColor range:range];
    }
    ...
    for (NSValue *value in userRanges) {
        NSRange range;
        [value getValue:&range];
        [self.textString addAttribute:NSForegroundColorAttributeName value:self.userHightColor range:range];
    }
    ...
    for (NSValue *value in topicRanges) {
        NSRange range;
        [value getValue:&range];
        [self.textString addAttribute:NSForegroundColorAttributeName value:self.topicHightColor range:range];
    }
    ...
    for (NSValue *value in agreementRanges) {
        NSRange range;
        [value getValue:&range];
        [self.textString addAttribute:NSForegroundColorAttributeName value:self.agreementHightColor range:range];
    }
    ...
    if (self.matchArr.count > 0) {
        ...
        if (userDefineRangeDicts.count > 0) {
            ...
            for (NSDictionary *dict in userDefineRangeDicts) {
                NSValue *value = dict[SDRange];
                [arrM addObject:value];
                UIColor *color = dict[SDColor];
                NSRange range;
                [value getValue:&range];
                [self.textString addAttribute:NSForegroundColorAttributeName value:color range:range];
            }
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    // 3.取出内容
    NSString *selectedString = [[self.textString string] substringWithRange:self.selectedRange];
    ...
    switch (self.ClickStyle) {
        case SDAttributeLabelStyleAgreement:{
            ...
            if (self.tapOperation) {
                ...
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStyleAgreement, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SDAttributeLabelStyleLink:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStyleLink, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SDAttributeLabelStyleTopic:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStyleTopic, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SDAttributeLabelStyleUser:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStyleUser, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SDAttributeLabelStyleUserDefine:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStyleUserDefine, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SDAttributeLabelStylePhoneNumber:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SDAttributeLabelStylePhoneNumber, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
    ...
    // 4.代理
    if ([self.delegate respondsToSelector:@selector(attributeLabel:didSelectedString:forStyle:onRange:)]) {
        
        [self.delegate attributeLabel:self didSelectedString:selectedString forStyle:self.ClickStyle onRange:self.selectedRange];
...
-(NSRange)getSelectRange:(CGPoint)selectPoint{
    ...
    if (self.textString.length == 0) return NSMakeRange(0, 0);
    ...
    for (NSValue *value in self.linkRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStyleLink;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStyleUser;
            return range;
        }
    }
    ...
    for (NSValue *value in self.topicRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStyleTopic;
            return range;
        }
    }
    ...
    for (NSValue *value in self.agreementRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStyleAgreement;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userDefineRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStyleUserDefine;
            return range;
        }
    }
    // 2.6.判断是否是一个电话号码
    for (NSValue *value in self.PhoneNumberRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = SDAttributeLabelStylePhoneNumber;
            return range;
        }
    }
...
-(NSArray<NSDictionary*> *)getUserDefineStringsRange{
    ...
    NSString *str = [self.textString string];
    for (NSDictionary *dict in self.matchArr) {
        ...
        aDictM[SDRange] = value;
        aDictM[SDColor] = color;
...
-(NSArray *)getRangesFromResult:(NSRegularExpression *)regex{
    ...
    NSArray<NSTextCheckingResult *> *results = [regex matchesInString:[self.textString string] options:0 range:NSMakeRange(0, self.textString.length)];
